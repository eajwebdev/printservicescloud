...
const TICKER = ['Full Sublimation', 'DTF', 'Shirts', 'Tarpaulin', 'Signages', 'Stickers', 'Logo', 'Decals', 'Laser Cutting', '3D/CAD/Blueprint'];

export default function Landing({ branches, signedIn }: Props) {
    return (
        <div data-theme="dark" className="relative min-h-dvh overflow-x-clip bg-[#07080A] font-sans text-[#ECEEF2] antialiased selection:bg-[#E4141B] selection:text-white">
            <Head title="Custom sublimation, DTF, signage and laser cutting">
                <meta name="description" content="SKC Custom Print: full sublimation, DTF, shirts, tarpaulin, signages, stickers, logo, decals, laser cutting and 3D/CAD/blueprint printing in Kabankalan, Dumaguete and Bacolod." />
            </Head>
            <Grain />
            <Nav signedIn={signedIn} />
            <Hero branchCount={branches.length} />
            <Ticker />
            <Services />
            <Process />
            <Branches branches={branches} />
            <Commitments branchCount={branches.length} />
            <Closing branches={branches} />
            <Footer branches={branches} signedIn={signedIn} />
        </div>
    );
}
...
function Eyebrow({ index, children, className }: { index?: string; children: ReactNode; className?: string }) {
    return (
        <p className={cn('flex items-center gap-3 font-mono text-[11px] tracking-[0.22em] text-[#8A93A3] uppercase', className)}>
            {index && <span className="text-[#E4141B]">{index}</span>}
    ...
    return (
        ...
        >
            <nav className="mx-auto flex h-[72px] max-w-[1320px] items-center gap-6 px-5 sm:px-8" aria-label="Main">
                <a href="#top" onClick={(e) => { e.preventDefault(); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="shrink-0" aria-label="SKC Custom Print, back to top">
                    <img src="/skclogo.png" alt="SKC Custom Print" className="h-10 w-auto" width={80} height={40} />
                </a>
                    ...
                    {LINKS.map((l) => (
                        <li key={l.id}>
                            <button type="button" onClick={() => scrollToId(l.id)} className="group relative rounded-full px-3.5 py-2 text-[13px] text-[#B4BAC5] transition-colors hover:text-white">
                                {l.label}
                                <span className="absolute inset-x-3.5 bottom-1 h-px origin-left scale-x-0 bg-[#E4141B] transition-transform duration-300 group-hover:scale-x-100" />
                            </button>
                        </li>
                    ))}
                </ul>
                            ...
                            {LINKS.map((l, i) => (
                                <motion.li key={l.id} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.05 * i }}>
                                    <button type="button" onClick={() => { setOpen(false); scrollToId(l.id); }} className="flex w-full items-center justify-between border-b border-white/[0.06] py-4 font-display text-2xl">
                                        {l.label}
                                        <ArrowRight className="size-5 text-[#E4141B]" />
                                    </button>
                                </motion.li>
                            ))}
                            <li className="pt-4">
...
function Hero({ branchCount }: { branchCount: number }) {
    ...
    return (
        <section id="top" ref={ref} onPointerMove={onMove} className="relative isolate flex min-h-[100svh] items-center overflow-hidden pt-[72px]">
            ...
            <motion.div
                className="absolute inset-0 -z-10 bg-[radial-gradient(rgba(228,20,27,0.55)_1.2px,transparent_1.6px)] [background-size:14px_14px]"
            ...
            <div className="absolute top-1/3 -right-40 -z-10 size-[620px] rounded-full bg-[#E4141B] opacity-[0.13] blur-[140px]" aria-hidden />
            ...
            >
                {[
                    { w: 'h-[16%]', c: 'bg-[#E4141B]/80', d: 0.35, top: 'top-[22%]' },
                    { w: 'h-[3%]', c: 'bg-white/80', d: 0.45, top: 'top-[46%]' },
                    { w: 'h-[8%]', c: 'bg-[#E4141B]/45', d: 0.55, top: 'top-[56%]' },
                ].map((s) => (
                    <motion.span
                        key={s.top}
                ...
                <motion.div style={{ y: copyY, opacity: fade }}>
                    <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.3, ease: EASE }} className="mb-8 flex items-start gap-3 font-mono text-[11px] leading-5 tracking-[0.22em] text-[#8A93A3] uppercase">
                        <span className="relative mt-1.5 flex size-2 shrink-0">
                            <span className="absolute inline-flex size-full animate-ping rounded-full bg-[#E4141B] opacity-60" />
                            <span className="relative inline-flex size-2 rounded-full bg-[#E4141B]" />
                        </span>
                        Custom print house &middot; {branchCount} branches across Negros
                    </motion.p>
                    ...
                    <h1 className="font-display text-[clamp(2.6rem,10.5vw,3.6rem)] leading-[0.95] font-semibold tracking-[-0.045em] sm:text-[clamp(3.6rem,7vw,5.4rem)] xl:text-[5.6rem]">
                        <RegisteredLine text="From jerseys" delay={0.45} />
                        <RegisteredLine text="to blueprints." delay={0.62} className="text-white" />
                        <span className="mt-3 block overflow-hidden">
                            <motion.span className="block text-[0.42em] leading-tight font-medium tracking-[-0.02em] text-[#8A93A3]" initial={{ y: '100%' }} animate={{ y: 0 }} transition={{ duration: 1, delay: 1.25, ease: EASE }}>
                                Printed right, <span className="text-[#E4141B]">made to be seen.</span>
                            </motion.span>
                        </span>
                    </h1>
                    ...
                    <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 1.4, ease: EASE }} className="mt-8 max-w-[34rem] text-[17px] leading-relaxed text-[#A3AAB6]">
                        Full sublimation, DTF, shirts, tarpaulins, signages, stickers, decals, laser cutting and 3D, CAD and blueprint work. Designed, proofed and produced in-house.
                    </motion.p>
                    ...
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 1.55, ease: EASE }} className="mt-10 flex flex-wrap items-center gap-3">
                        <MagneticButton onClick={() => scrollToId('services')} primary>
                            See what we make <ArrowRight className="size-4" />
                        </MagneticButton>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.8, duration: 1 }} className="absolute inset-x-0 bottom-6 mx-auto hidden max-w-[1320px] items-center justify-between px-8 font-mono text-[10px] tracking-[0.2em] text-[#5B6472] uppercase md:flex">
                <span className="flex items-center gap-2">
                    ...
                    <span className="ml-2">C 0 / M 100 / Y 100 / K 0</span>
                </span>
                <button type="button" onClick={() => scrollToId('services')} className="flex items-center gap-3 hover:text-[#ECEEF2]">
                    ...
                    <span className="relative h-8 w-px overflow-hidden bg-[#2A2F3A]">
                        <motion.span className="absolute inset-x-0 top-0 h-3 bg-[#E4141B]" animate={{ y: [-12, 32] }} transition={{ duration: 1.6, repeat: Infinity, ease: 'easeInOut' }} />
                    </span>
                </button>
            </motion.div>
        </section>
...
/** The SKC mark pressed onto the page, with the service list spinning around it like a stamp. */
function HeroMark() {
    const ring = 'FULL SUBLIMATION • DTF • SIGNAGES • DECALS • LASER CUTTING • BLUEPRINT • ';
    return (
    ...
    return (
        <motion.button
            ...
            className={cn(
                ...
                primary ? 'bg-[#E4141B] text-white shadow-[0_18px_40px_-14px_rgba(228,20,27,0.75)]' : 'border border-white/15 text-white hover:border-white/40',
            )}
        >
                    ...
                    <div key={copy} className="flex shrink-0 items-center" aria-hidden={copy === 1}>
                        {items.map((t, i) => (
                            <span key={`${copy}-${i}`} className="flex items-center">
                                ...
                                <span className={cn('size-3 rotate-45', outline ? 'border border-[#3A4150]' : 'bg-[#E4141B]')} />
                            </span>
                        ))}
                    </div>
                ))}
...
const SERVICES: ServiceCard[] = [
    { no: '01', title: 'Full Sublimation', body: 'Edge-to-edge jerseys, polos and mugs. The color is dyed into the fabric, so it won’t crack or peel.', tags: ['Jerseys', 'Polo shirts', 'Mugs'], art: (a) => <ArtSublimation active={a} />, span: 'lg:col-span-7 lg:row-span-2' },
    { no: '02', title: 'DTF', body: 'Direct-to-film transfers in full color for cotton and blends, sharp even on dark shirts.', tags: ['Transfers', 'Shirt prints'], art: (a) => <ArtDtf active={a} />, span: 'lg:col-span-5' },
    { no: '03', title: 'Shirts', body: 'Team, event and one-off shirts in DTF, sublimation or heat transfer vinyl.', tags: ['Events', 'Uniforms'], art: (a) => <ArtShirt active={a} />, span: 'lg:col-span-5' },
    { no: '04', title: 'Tarpaulin', body: 'From a birthday greeting to a full fiesta street run, finished with eyelets, rope or frames.', tags: ['Banners', 'Backdrops'], art: (a) => <ArtTarp active={a} />, span: 'lg:col-span-4' },
    { no: '05', title: 'Signages', body: 'Sintra boards and laser-cut acrylic signs with LED backlight, built and installed.', tags: ['Acrylic', 'Sintra', 'LED'], art: (a) => <ArtSign active={a} />, span: 'lg:col-span-4' },
    { no: '06', title: 'Stickers', body: 'Die-cut, clear and laminated stickers for products, packaging and labels.', tags: ['Die-cut', 'Labels'], art: (a) => <ArtSticker active={a} />, span: 'lg:col-span-4' },
    { no: '07', title: 'Logo', body: 'New logos and redraws, delivered as clean vectors that print sharp at any size.', tags: ['Design', 'Vector'], art: (a) => <ArtLogo active={a} />, span: 'lg:col-span-4' },
    { no: '08', title: 'Decals', body: 'Cut and reflective vinyl for cars, motorcycles, trucks and storefront glass.', tags: ['Vehicles', 'Reflective'], art: (a) => <ArtDecal active={a} />, span: 'lg:col-span-4' },
    { no: '09', title: 'Laser Cutting', body: 'Clean cuts and engraving on acrylic and plywood: keychains, plaques and letters.', tags: ['Acrylic', 'Wood', 'Engraving'], art: (a) => <ArtLaser active={a} />, span: 'lg:col-span-4' },
    { no: '10', title: '3D / CAD / Blueprint', body: 'Plan and blueprint printing, CAD drafting, 3D modeling and PLA prints.', tags: ['Plans', 'CAD', '3D print'], art: (a) => <ArtBlueprint active={a} />, span: 'lg:col-span-12' },
];
            ...
            <div className="mb-16 grid gap-8 lg:mb-20 lg:grid-cols-[1fr_1fr] lg:items-end">
                <div>
                    ...
                    <h2 className="mt-6 font-display text-[clamp(2.4rem,5.2vw,4.6rem)] leading-[0.95] font-semibold tracking-[-0.04em]">
                        <Reveal>Ten ways to</Reveal>
                        <Reveal delay={0.08}>
                            put your name <span className="text-[#E4141B]">out there.</span>
                        </Reveal>
                    </h2>
                </div>
                <Reveal delay={0.15} className="lg:justify-self-end">
                    <p className="max-w-md text-[16px] leading-relaxed text-[#8A93A3]">One shop floor for fabric, film, vinyl, acrylic, wood and paper. Bring a finished file, a sketch or just the idea.</p>
                </Reveal>
            </div>

            <div className="grid auto-rows-[minmax(300px,auto)] gap-3 md:grid-cols-2 lg:grid-cols-12">
                {SERVICES.map((s, i) => (
                    <ServiceTile key={s.no} service={s} index={i} />
                ))}
            </div>
        </section>
    );
...
function ServiceTile({ service, index }: { service: ServiceCard; index: number }) {
    ...
    const glow = useMotionTemplate`radial-gradient(420px circle at ${px}% ${py}%, rgba(228,20,27,0.16), transparent 60%)`;
    ...
    return (
        <motion.article
            ...
            className={cn('group relative flex flex-col overflow-hidden rounded-[22px] border border-white/[0.07] bg-[#0D0F13] p-7 sm:p-8', service.span)}
        >
            ...
            <div className="relative flex items-start justify-between gap-4">
                <span className="font-mono text-[11px] tracking-[0.2em] text-[#5B6472]">{service.no}</span>
                <span className="grid size-9 place-items-center rounded-full border border-white/10 text-[#8A93A3] transition-all duration-500 group-hover:rotate-45 group-hover:border-[#E4141B] group-hover:bg-[#E4141B] group-hover:text-white">
                    <ArrowUpRight className="size-4" />
                </span>
            </div>
            <div className="relative my-6 flex min-h-[140px] flex-1 items-center justify-center">{service.art(hover || false)}</div>
            <div className="relative">
                <h3 className="font-display text-[26px] leading-tight font-semibold tracking-[-0.02em] sm:text-[30px]">{service.title}</h3>
                <p className="mt-2 max-w-[46ch] text-[15px] leading-relaxed text-[#8A93A3]">{service.body}</p>
                <ul className="mt-5 flex flex-wrap gap-1.5">
                    {service.tags.map((t) => (
                        <li key={t} className="rounded-full border border-white/[0.08] px-2.5 py-1 font-mono text-[10px] tracking-[0.12em] text-[#8A93A3] uppercase">
                            {t}
                        </li>
                    ))}
                </ul>
        ...
        <svg viewBox="0 0 320 220" className="h-full max-h-[300px] w-full" aria-hidden>
            <defs>
                <linearGradient id="sub-dye" x1="0" x2="1" y1="0" y2="1">
                    <stop offset="0" stopColor="#E4141B" />
                    <stop offset="0.45" stopColor="#7A0B10" />
                    <stop offset="0.55" stopColor="#ECEEF2" />
                    <stop offset="1" stopColor="#E4141B" />
                </linearGradient>
...
function ArtDtf({ active }: { active: boolean }) {
    return (
        <svg viewBox="0 0 240 150" className="h-[150px] w-full" aria-hidden>
            ...
            <g>
                <circle cx="120" cy="66" r="22" fill="#E4141B" />
...
function ArtShirt({ active }: { active: boolean }) {
    return (
        <svg viewBox="0 0 240 150" className="h-[150px] w-full" aria-hidden>
            ...
            {['#ECEEF2', '#E4141B', '#8A93A3'].map((c, i) => (
                <motion.rect key={c} x={96 + i * 16} y="60" width="14" height="34" rx="2" fill={c} animate={{ scaleY: active ? [0.2, 1] : 0.7, opacity: active ? 1 : 0.55 }} style={{ originY: 1 }} transition={{ duration: 0.5, delay: active ? i * 0.1 : 0, ease: EASE }} />
            ))}
    ...
    return (
        <svg viewBox="0 0 240 150" className="h-[150px] w-full" aria-hidden>
            <motion.g animate={active ? { skewY: [0, -2, 1.5, 0] } : { skewY: 0 }} transition={{ duration: 1.4, repeat: active ? Infinity : 0, ease: 'easeInOut' }}>
                ...
                <rect x="28" y="36" width="184" height="18" fill="#E4141B" />
        ...
        <svg viewBox="0 0 240 150" className="h-[150px] w-full" aria-hidden>
            ...
            {letters.map((l, i) => (
                <motion.text
                    ...
                    fontWeight="700"
                    fontSize="40"
                    animate={{ fill: active ? '#FF3B41' : '#3A4150', filter: active ? 'drop-shadow(0 0 8px #E4141B)' : 'drop-shadow(0 0 0 transparent)' }}
...
function ArtSticker({ active }: { active: boolean }) {
    return (
        <svg viewBox="0 0 240 150" className="h-[150px] w-full" aria-hidden>
            ...
            <circle cx="120" cy="75" r="44" fill="#E4141B" />
            <text x="120" y="84" textAnchor="middle" style={{ fontFamily: 'var(--font-display)' }} fontWeight="700" fontSize="26" fill="#ECEEF2">
                SKC
            </text>
            <motion.path
...
function ArtLaser({ active }: { active: boolean }) {
    ...
    return (
        <svg viewBox="0 0 240 150" className="h-[150px] w-full" aria-hidden>
            ...
            <motion.path d={path} fill="none" stroke="#FF5A5F" strokeWidth="2" style={{ pathLength: draw, filter: 'drop-shadow(0 0 4px #E4141B)' }} />
            <motion.circle r="4" fill="#FFF" style={{ cx, cy, filter: 'drop-shadow(0 0 6px #E4141B)' }} animate={{ opacity: active ? 1 : 0 }} />
                ...
                <div className="mx-auto w-full max-w-[1320px] px-5 py-28 sm:px-8 lg:py-0">
                    <div className="mb-14 flex flex-wrap items-end justify-between gap-6">
                        <div>
                            ...
                            <h2 className="mt-6 font-display text-[clamp(2.4rem,5.2vw,4.6rem)] leading-[0.95] font-semibold tracking-[-0.04em]">
                                <Reveal>No guesswork</Reveal>
                                <Reveal delay={0.08}>between order and pickup.</Reveal>
                            </h2>
                        </div>
                        <p className="hidden font-mono text-[11px] tracking-[0.2em] text-[#5B6472] uppercase lg:block">
                            Step <span className="text-[#ECEEF2]">{String(step + 1).padStart(2, '0')}</span> / 04
                        </p>
                    </div>
                    ...
                    <div className="relative hidden lg:block">
                        <div ref={board} className="grid grid-cols-4 gap-3">
                            {STEPS.map((s, i) => (
                                <div key={s.title} className={cn('relative min-h-[360px] rounded-[20px] border p-6 transition-colors duration-500', i <= step ? 'border-white/[0.1] bg-[#0F1116]' : 'border-white/[0.04] bg-transparent')}>
                                    <div className="flex items-center justify-between font-mono text-[11px] tracking-[0.2em] uppercase">
                                        <span className={i <= step ? 'text-[#E4141B]' : 'text-[#3A4150]'}>0{i + 1}</span>
                                        <span className={cn('size-1.5 rounded-full transition-colors', i === step ? 'bg-[#E4141B]' : i < step ? 'bg-[#8A93A3]' : 'bg-[#2A2F3A]')} />
                                    </div>
                                    <h3 className={cn('mt-4 font-display text-[24px] font-semibold tracking-[-0.02em] transition-colors duration-500', i <= step ? 'text-white' : 'text-[#3A4150]')}>{s.title}</h3>
                                    <p className={cn('mt-2 text-[14px] leading-relaxed transition-colors duration-500', i <= step ? 'text-[#8A93A3]' : 'text-[#2A2F3A]')}>{s.body}</p>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="pointer-events-none absolute inset-x-0 bottom-10">
                            <motion.div className="w-[calc((100%-36px)/4)] px-5" style={{ x: ticketX }}>
                                <JobTicket stage={STEPS[step].tag} />
                            </motion.div>
                        </div>
                        <div className="mt-6 h-px w-full bg-white/[0.06]">
                            <motion.div className="h-px bg-[#E4141B]" style={{ width: bar }} />
                        </div>
                    </div>
                    ...
                    <ol className="space-y-3 lg:hidden">
                        {STEPS.map((s, i) => (
                            <ProcessItem key={s.title} index={i} title={s.title} body={s.body} />
                        ))}
                    </ol>
                </div>
            </div>
...
function ProcessItem({ index, title, body }: { index: number; title: string; body: string }) {
    ...
    return (
        <motion.li ref={ref} initial={{ opacity: 0, x: -20 }} animate={inView ? { opacity: 1, x: 0 } : undefined} transition={{ duration: 0.7, ease: EASE }} className="flex gap-5 rounded-[18px] border border-white/[0.08] bg-[#0F1116] p-6">
            <span className="font-mono text-[12px] text-[#E4141B]">0{index + 1}</span>
            <div>
...
function JobTicket({ stage }: { stage: string }) {
    return (
        <div className="relative rounded-[14px] bg-[#ECEEF2] p-4 text-[#07080A] shadow-[0_30px_60px_-20px_rgba(0,0,0,0.8)]">
            <div className="flex items-center justify-between font-mono text-[10px] tracking-[0.14em]">
                <span>KAB-2609-0142</span>
                <span className="rounded-full bg-[#E4141B] px-2 py-0.5 text-white">RUSH</span>
            </div>
            <p className="mt-2 font-display text-[16px] leading-tight font-semibold">15× Full sublimation jerseys</p>
            <p className="text-[12px] text-[#5B6472]">Name &amp; number, league set</p>
            <div className="mt-3 flex items-center justify-between border-t border-dashed border-[#9AA2B1] pt-2.5">
                <AnimatePresence mode="wait">
                    <motion.span key={stage} initial={{ y: 8, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -8, opacity: 0 }} transition={{ duration: 0.25 }} className="font-mono text-[10px] font-semibold tracking-[0.14em] text-[#E4141B]">
                        {stage}
                    </motion.span>
                </AnimatePresence>
...
/* -------------------------------------------------------------------------- */
/* Branches: Negros Island with every SKC shop pinned                          */
/* -------------------------------------------------------------------------- */
            ...
            <div className="grid gap-14 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
                <div ref={mapRef} className="relative order-2 lg:order-1">
                    <div className="relative mx-auto aspect-[460/700] w-full max-w-[440px]">
                        ...
                        <svg viewBox="0 0 460 700" className="h-full w-full" role="img" aria-label={`Map of Negros Island with ${pinned.length} SKC Custom Print branches`}>
                            <defs>
                ...
                <div className="order-1 lg:order-2">
                    ...
                    <h2 className="mt-6 font-display text-[clamp(2.4rem,5.2vw,4.6rem)] leading-[0.95] font-semibold tracking-[-0.04em]">
                        <Reveal>Same standard,</Reveal>
                        <Reveal delay={0.08}>
                            <span className="text-[#E4141B]">{branches.length}</span> shops on Negros.
                        </Reveal>
                    </h2>
                    <p className="mt-6 max-w-lg text-[16px] leading-relaxed text-[#8A93A3]">Walk in, send your file, or call ahead. Every branch runs the same price list, the same proofing and the same job tracking.</p>
...
function BranchRow({ branch, index, onFocus, active }: { branch: BranchInfo; index: number; onFocus: (code: string | null) => void; active: boolean }) {
    const ref = useRef<HTMLLIElement>(null);
    const inView = useInView(ref, { once: true, margin: '-10% 0px' });
    const city = branch.name.replace(/^SKC\s+/i, '');
    return (
        <motion.li
            ref={ref}
        ...
        >
            <motion.span className="absolute inset-y-0 -left-5 w-[3px] origin-top bg-[#E4141B]" animate={{ scaleY: active ? 1 : 0 }} transition={{ duration: 0.35, ease: EASE }} aria-hidden />
            <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-2">
                <h3 className="font-display text-[clamp(1.8rem,3.4vw,2.7rem)] leading-none font-semibold tracking-[-0.03em]">
                    <span className="text-[#5B6472] transition-colors group-hover:text-[#E4141B]">SKC </span>
                    {city}
                </h3>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
                {branch.phone && (
                    <a href={`tel:${branch.phone.replace(/\s+/g, '')}`} className="inline-flex items-center gap-2 rounded-full border border-white/10 px-3.5 py-1.5 text-[13px] text-[#ECEEF2] transition-colors hover:border-[#E4141B] hover:bg-[#E4141B]">
                ...
                {branch.address && (
                    <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`SKC Custom Print ${branch.address}`)}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[13px] text-[#8A93A3] hover:text-white">
...
function Commitments({ branchCount }: { branchCount: number }) {
    const items: { big: ReactNode; label: string; body: string }[] = [
        { big: <CountTo value={10} />, label: 'Print disciplines', body: 'Fabric, film, vinyl, acrylic, wood and paper under one roof.' },
        { big: <CountTo value={branchCount} />, label: 'Branches', body: 'Kabankalan, Dumaguete and Bacolod, on one system.' },
    ...
    return (
        <section className="border-y border-white/[0.06] bg-[#0A0B0E]">
            <div className="mx-auto grid max-w-[1320px] sm:grid-cols-2 lg:grid-cols-4">
                {items.map((it, i) => (
                    <PromiseCell key={it.label} index={i} {...it} />
                ))}
            </div>
        </section>
    );
...
function PromiseCell({ big, label, body, index }: { big: ReactNode; label: string; body: string; index: number }) {
    ...
    return (
        <motion.div ref={ref} initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : undefined} transition={{ duration: 0.8, delay: index * 0.1 }} className="border-white/[0.06] px-8 py-12 sm:border-r sm:[&:nth-child(2n)]:border-r-0 lg:[&:nth-child(2n)]:border-r lg:last:border-r-0">
            ...
            <p className="mt-4 font-mono text-[11px] tracking-[0.2em] text-[#E4141B] uppercase">{label}</p>
...
function Closing({ branches }: { branches: BranchInfo[] }) {
    const ref = useRef<HTMLElement>(null);
    ...
    return (
        <section id="contact" ref={ref} className="relative scroll-mt-20 overflow-hidden py-32 lg:py-44">
            ...
            <div className="relative mx-auto max-w-[1320px] px-5 sm:px-8">
                <Eyebrow index="04">Start a job</Eyebrow>
                <h2 className="mt-8 max-w-5xl font-display text-[clamp(2.8rem,7vw,6.6rem)] leading-[0.92] font-semibold tracking-[-0.05em]">
                    <Reveal>Got a design in mind?</Reveal>
                    <Reveal delay={0.1}>
                        <span className="text-[#5B6472]">Bring it to the </span>
                        <span className="relative inline-block">
                            nearest SKC.
                            <motion.span className="absolute right-0 -bottom-1 left-0 h-[0.08em] origin-left bg-[#E4141B]" initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true }} transition={{ duration: 1, delay: 0.6, ease: EASE }} />
                        </span>
                    </Reveal>
                </h2>
                <div className="mt-12 flex flex-wrap gap-3">
                    {first?.phone && (
                        <a href={`tel:${first.phone.replace(/\s+/g, '')}`} className="group relative inline-flex h-14 items-center gap-3 overflow-hidden rounded-full bg-[#E4141B] px-8 text-[15px] font-medium text-white">
...
function Footer({ branches, signedIn }: { branches: BranchInfo[]; signedIn: boolean }) {
    return (
        <footer className="relative border-t border-white/[0.06] bg-[#050607]">
            <div className="mx-auto grid max-w-[1320px] gap-12 px-5 py-16 sm:px-8 md:grid-cols-[1.3fr_1fr_1fr]">
                <div>
                    <img src="/skclogo.png" alt="SKC Custom Print" className="h-16 w-auto" width={128} height={64} />
                    <p className="mt-5 max-w-xs text-[14px] leading-relaxed text-[#5B6472]">Full sublimation, DTF, shirts, tarpaulin, signages, stickers, logo, decals, laser cutting and 3D/CAD/blueprint.</p>
                </div>
                <div>
            ...
            <div className="border-t border-white/[0.06]">
                <div className="mx-auto flex max-w-[1320px] flex-wrap items-center justify-between gap-4 px-5 py-6 font-mono text-[11px] tracking-[0.14em] text-[#5B6472] uppercase sm:px-8">
                    <span>&copy; {new Date().getFullYear()} SKC Custom Print</span>
